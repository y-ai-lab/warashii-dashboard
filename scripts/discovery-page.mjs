...
function promotedCard(x) {
  const radar = x.radar_id ? `${BASE}/opportunities/${encodeURIComponent(x.radar_id)}.html` : `${BASE}/`;
  const unresolved = (x.unresolved ?? []).map(v => `<li>${esc(v)}</li>`).join('');
  return `<article class="card"><div class="card-top"><div><div class="badges"><span class="badge rec" data-rec="WATCH">WATCH</span><span class="badge category">Radar昇格済み</span><span class="badge freshness">${esc(x.promoted_at ?? x.reviewed_at)}</span></div><h2 class="title">${esc(x.title)}</h2><p class="provider">Discovery → Radar</p></div><div class="score"><strong>${esc(x.score ?? '-')}</strong><span>/100</span></div></div><div class="reward">${esc(x.reward_label ?? '価値未確定')}</div><dl class="facts"><div><dt>必要資金</dt><dd>${Number(x.required_funds_yen ?? 0)===0?'¥0':`¥${esc(x.required_funds_yen)}`}</dd></div><div><dt>Risk</dt><dd>${esc(x.risk_label)}</dd></div><div><dt>スマホ</dt><dd>${esc(x.mobile)}</dd></div><div><dt>判定</dt><dd>${esc(x.decision ?? 'WATCH')}</dd></div></dl><p class="conversion">変換ルート：${esc(x.conversion_route)}</p><p class="notes">${esc(x.review_note)}</p>${unresolved?`<details><summary>昇格後も残る未確定事項</summary><ul class="conditions">${unresolved}</ul></details>`:''}<div class="card-footer"><a class="source" href="${esc(radar)}">Radar詳細を見る</a></div></article>`;
}
...
function render(queue, summary) {
  ...
  const promoted = all.filter(x => x.status === 'PROMOTED_WATCH');
  const p = pending.length ? pending.map(pendingCard).join('\n') : '<p class="empty">現在、未検証のレビュー待ち候補はありません。</p>';
  const r = reviewed.length ? reviewed.map(reviewedCard).join('\n') : '<p class="empty">現在、レビュー済みで昇格保留のWATCH候補はありません。</p>';
  const m = promoted.length ? promoted.map(promotedCard).join('\n') : '<p class="empty">まだDiscoveryからRadarへ昇格した候補はありません。</p>';
  return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="わらしべ Asset RadarのOpportunity Discovery。未検証候補、公式確認後のWATCH、Radar昇格履歴を分離して公開。"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${BASE}/discovery/"><link rel="alternate" type="application/rss+xml" title="わらしべ Asset Radar 更新フィード" href="${BASE}/feed.xml"><meta property="og:type" content="website"><meta property="og:locale" content="ja_JP"><meta property="og:site_name" content="わらしべ Asset Radar"><meta property="og:title" content="Opportunity Discovery｜わらしべ Asset Radar"><meta property="og:description" content="自動発見、レビュー、Radar昇格までの履歴を分離管理。"><meta property="og:url" content="${BASE}/discovery/"><meta name="twitter:card" content="summary"><title>Opportunity Discovery｜わらしべ Asset Radar</title><link rel="stylesheet" href="../styles.css"></head><body><header class="site-header"><div class="shell header-inner"><div><p class="eyebrow">OPPORTUNITY DISCOVERY</p><h1>候補発見・レビューキュー</h1><p class="subtitle">発見と採用を分離し、未確定案件をRadar本体へ混ぜない。</p></div><a class="header-badge" href="../">Radarへ戻る</a></div></header><main class="shell"><nav class="asset-nav"><a href="../">全案件</a><a href="../rankings/zero-yen.html">0円ランキング</a><a href="../categories/">カテゴリ</a><a href="../methodology/">評価方法</a><a href="../updates/">更新履歴</a></nav><section class="score-guide" style="border-width:2px"><h2>⚠ 発見シグナル ≠ 推薦</h2><p><b>REVIEW_REQUIRED は実行禁止。</b> REVIEWED_WATCHも不確定条件が残るため、Radar本体への昇格・実行推奨とは別扱いです。PROMOTED_WATCHは公式確認を経てRadar本体のWATCHへ移した履歴です。</p></section><section class="ranking-hero"><p class="eyebrow">DISCOVERY STATUS</p><h2>候補発見エンジン</h2><p>プラットフォームやGitHubは発見用にのみ使い、判断はプロジェクト自身の公式情報へ戻します。</p><div class="ranking-meta"><span>未検証 <b>${pending.length}</b></span><span>レビュー済WATCH <b>${reviewed.length}</b></span><span>Radar昇格 <b>${promoted.length}</b></span><span>今回新規 <b>${esc(summary.new_count??0)}</b></span><span>Hard Gate除外 <b>${esc(summary.hard_gate_filtered??0)}</b></span><span>ノイズ/Utility除外 <b>${esc((summary.navigation_filtered??0)+(summary.utility_filtered??0))}</b></span><span>エラー <b>${esc(summary.error_count??0)}</b></span><span>最終探索 <b>${esc(summary.generated_at??queue.generated_at??'未実行')}</b></span></div></section><section class="score-guide"><h2>未検証候補</h2><p>実行禁止の隔離領域です。</p></section><section class="cards">${p}</section><section class="score-guide"><h2>レビュー済みWATCH</h2><p>公式確認済みでも、価値・地域・受領条件などが未確定でRadar昇格を保留している候補です。</p></section><section class="cards">${r}</section><section class="score-guide"><h2>Radar昇格履歴</h2><p>Discoveryで発見され、公式確認・100点評価を経てRadar本体へ移した候補です。</p></section><section class="cards">${m}</section><section class="score-guide"><h2>Radar採用までのゲート</h2><ol class="conditions"><li>公式サイト / Docs / 公式SNSを特定</li><li>0円開始と0円継続を分離確認</li><li>地域・KYC・資金・規約・Hard Gateを確認</li><li>100点評価でGO/WATCH/STOPを決定</li><li>必要条件が十分確認できたものだけRadar本体へ追加</li></ol></section></main><footer class="shell footer"><p>Discoveryは候補探索の情報資産です。未確定情報をRadar本体から隔離し、昇格履歴まで追跡します。</p></footer></body></html>`;
}
